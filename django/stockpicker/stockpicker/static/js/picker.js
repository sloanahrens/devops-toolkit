(function($){
  ...
  var App = {
    ...
    formatRecommendation: function(obj) {
      return '<tr>' +
        '<td>' +
        '<a href="#" data-ticker="' + obj['symbol'] + '">' + obj['symbol'] + '</a>' +
        ' [<a target="_blank" href="https://www.google.com/finance?q=' + obj['symbol'] + '">gf</a>]</td>' +
        '<td>' + obj['ac']  + '</td>' +
        '<td>' + obj['sac']  + '</td>' +
        '<td>' + obj['sac_ma']  + '</td>' +
        '<td>' + obj['ratio']  + '</td>' +
    ...
    displayRecommendations: function(json, startingLetter) {
      ...
      for(var i = 0; i < json['sell_hits'].length; i++){
        ...
        if(startingLetter == null || startingLetter == hit['symbol'][0]){
    ...
    getRecommendations: function() {
      ...
      var deferredRequest = $.getJSON("/tickers/recommendations/");
      ...
      deferredRequest.done(function(json) {
        ...
        for(var i = 0; i < json['sell_hits'].length; i++){
          var hit = json['sell_hits'][i];
          startingLetters[hit['symbol'][0]] = null;
        }
        for(var i = 0; i < json['buy_hits'].length; i++){
          var hit = json['buy_hits'][i];
          startingLetters[hit['symbol'][0]] = null;
    ...
    getTickerList: function() {
      ...
      var deferredRequest = $.getJSON("/tickers/tickerlist");
